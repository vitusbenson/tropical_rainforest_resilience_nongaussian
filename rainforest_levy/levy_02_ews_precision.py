...
if __name__ == "__main__":
    ...
    df = pd.read_csv(f"../data/amaz_ts_20221202/ts_t_climatechange={t_climatechange}-alpha={sim_kwargs['alpha']}-sigma={sim_kwargs['sigma']}-only_negative_disturbances={sim_kwargs['only_negative_disturbances']}-add_short_timescale={sim_kwargs['add_short_timescale']}.csv", index_col = 0)
    #curr_df = df[["Ps", "Ts_det", "Tfixs", "Ts_0", "Ts_1"]]

    all_ews, pvals, coefs = compute_ews_from_df(df)

    for ews in all_ews:
        savepath = Path("../data/amaz_ews_20221202/")/f"seed={ews['seed']}"
    ...
    pvals.to_csv(f"../data/amaz_ews_20221202/pvals_t_climatechange={t_climatechange}-alpha={sim_kwargs['alpha']}-sigma={sim_kwargs['sigma']}-only_negative_disturbances={sim_kwargs['only_negative_disturbances']}-add_short_timescale={sim_kwargs['add_short_timescale']}.csv")

    coefs.to_csv(f"../data/amaz_ews_20221202/coefs_t_climatechange={t_climatechange}-alpha={sim_kwargs['alpha']}-sigma={sim_kwargs['sigma']}-only_negative_disturbances={sim_kwargs['only_negative_disturbances']}-add_short_timescale={sim_kwargs['add_short_timescale']}.csv")