...
# %%
recall = pd.read_csv("../data/share/recall_dw.csv", index_col=0)
# %%
...
sns.heatmap(recall[recall.only_negative_disturbances & (recall.ews == "iqr") & (recall.valid > 0.5)].pivot(index = "alpha", columns = "sigma", values = "p_kendalltau_adj"), vmin = 0.0, vmax = 1.0, cmap = "crest", ax = axs[0], cbar=False)
...
axs[0].set_title("IQR, Adj. Kendall-Tau slopes", fontdict={"fontsize":12})
...
sns.heatmap(recall[recall.only_negative_disturbances & (recall.ews == "std_dev") & (recall.valid > 0.5)].pivot(index = "alpha", columns = "sigma", values = "p_linear"), vmin = 0.0, vmax = 1.0, cmap = "crest", ax = axs[1], cbar=True, cbar_kws={"label": "Recall"})
...
axs[1].set_title("Std. Dev., Linear slopes", fontdict={"fontsize":12})
...
fig.suptitle("Double-Well Potential")
fig.tight_layout()
plt.savefig("..plots/fig3_dw.pdf", dpi = 300, bbox_inches = "tight", transparent = True)
# %%
