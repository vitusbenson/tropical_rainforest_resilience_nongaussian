...
if __name__ == "__main__":
    ...
    ews_path = f"../data/dw_ews_20230717/seed=0/ews_t_climatechange={t_climatechange}-alpha={sim_kwargs['alpha']}-sigma={sim_kwargs['sigma']}-only_negative_disturbances={sim_kwargs['only_negative_disturbances']}-seed=0.csv"
    #curr_df = df[["Ps", "Ts_det", "Tfixs", "Ts_0", "Ts_1"]]

    pvals, coefs = compute_falseposrate_from_df(ews_path, n_seeds=100)

    
    pvals.to_csv(f"../data/dw_ews_falseposrate_20230717/pvals_t_climatechange={t_climatechange}-alpha={sim_kwargs['alpha']}-sigma={sim_kwargs['sigma']}-only_negative_disturbances={sim_kwargs['only_negative_disturbances']}.csv")

    coefs.to_csv(f"../data/dw_ews_falseposrate_20230717/coefs_t_climatechange={t_climatechange}-alpha={sim_kwargs['alpha']}-sigma={sim_kwargs['sigma']}-only_negative_disturbances={sim_kwargs['only_negative_disturbances']}.csv")