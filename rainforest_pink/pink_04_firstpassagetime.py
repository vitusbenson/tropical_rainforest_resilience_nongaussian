    ...
    for sigma in tqdm(sigmas, position=1, desc = "sigma", leave=False):
        for only_negative_disturbances in [False, True]:
            for add_short_timescale in [False, True]:
                ...
                df = pd.read_csv(f"../data/amazpink_ts_20230712/ts_t_climatechange={t_climatechange}-alpha={alpha}-sigma={sigma}-only_negative_disturbances={only_negative_disturbances}-add_short_timescale={add_short_timescale}.csv", index_col = 0)
...
FPTs.to_csv("../data/amazpink_ews_20230712/fpts.csv")
# %%
