...
def simulate_T(T_start = 75.57915418, P_start = 5.0, P_end = 2.3, t_steadystate = 1000, t_climatechange = 500, t_afterchange = 500, dt = 0.01, alpha = 2.0, sigma = 0.02, only_negative_disturbances = False, add_short_timescale = True, seeds = [42, 69, 420]):
    ...
    for seed in seeds:
        ...
        # X_white = np.fft.rfft(rng.standard_normal(N_steps))
        # S = np.fft.rfftfreq(N_steps)
        # S = S / np.where(S == 0, float('inf'), np.sqrt(S))
        # S = S / np.sqrt(np.mean(S**2))
        # X_shaped = X_white * S
        # L_levy = np.fft.irfft(X_shaped)
        L_levy = pink_noise(N_steps)

        if only_negative_disturbances:
            L_levy = (L_levy < 0) * L_levy
...
def save_one_simulation(t_climatechange = 500, alpha = 2.0, sigma = 0.02, only_negative_disturbances = False, add_short_timescale = True, seeds = [42, 69, 420]):
    ...
    data.to_csv(f"../data/amazpink_ts_20230712/ts_t_climatechange={t_climatechange}-alpha={alpha}-sigma={sigma}-only_negative_disturbances={only_negative_disturbances}-add_short_timescale={add_short_timescale}.csv")
# %%
