...
def adjust_pval_row(row):
    ...
    ews_path = f"../data/amaz_ews_20221202/seed={row['seed']}/ews_t_climatechange={row['t_climatechange']}-alpha={row['alpha']}-sigma={sigma}-only_negative_disturbances={row['only_negative_disturbances']}-add_short_timescale={row['add_short_timescale']}-seed={row['seed']}.csv"
...
if __name__ == "__main__":
    ...
    all_pvals = []
    for path in Path("../data/amazpink_ews_20230712/").glob("pvals*.csv"):
        t_climatechange = int(path.name.split("=")[1].split("-")[0])
        ...
        add_short_timescale = (path.name.split("=")[5].split(".")[0]) == "True"
        ...
        curr_pvals["add_short_timescale"] = add_short_timescale
        all_pvals.append(curr_pvals)

    pvals = pd.concat(all_pvals, axis = 0)
    pvals.reset_index(drop = True).to_csv("../data/amazpink_ews_20230712/pvals.csv")


    all_pvals = []
    for path in Path("../data/amazpink_ews_falseposrate_20230712/").glob("pvals*.csv"):
        t_climatechange = int(path.name.split("=")[1].split("-")[0])
        alpha = float(path.name.split("=")[2].split("-")[0])
        sigma = float(path.name.split("=")[3].split("-only")[0])
        only_negative_disturbances = (path.name.split("=")[4].split("-")[0]) == "True"
    ...
    pvals = pd.concat(all_pvals, axis = 0)
    pvals.reset_index(drop = True).to_csv("../data/amazpink_ews_falseposrate_20230712/pvals.csv")


    print("Recall")
    pvals = pd.read_csv("../data/amazpink_ews_20230712/pvals.csv", index_col= 0)
    ...
    pvals3.to_csv("../data/amazpink_ews_20230712/pvals2.csv")

    print("FPR")
    pvals = pd.read_csv("../data/amazpink_ews_falseposrate_20230712/pvals.csv", index_col= 0)
    ...
    pvals3.to_csv("../data/amazpink_ews_falseposrate_20230712/pvals2.csv")

    # %%
    pvals = pd.read_csv("../data/amazpink_ews_20230712/pvals2.csv", index_col= 0)

    # %%
    fpts = pd.read_csv("../data/amazpink_ews_20230712/fpts.csv", index_col = 0)
    ...
    # %%
    pvals_plot.to_csv("../data/share/recall_pink.csv")
    # %%
    pvals = pd.read_csv("../data/amazpink_ews_falseposrate_20230712/pvals2.csv", index_col= 0)
    ...
    # %%
    pvals_plot.to_csv("../data/share/falseposrate_pink.csv")
# %%
